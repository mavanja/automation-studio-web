      ...
      <div className="p-7">
        {templates.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {templates.map(t => (
              <div key={t.id} className="bg-white border border-[#e2e5f0] rounded-[14px] shadow-sm p-5 flex flex-col">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="text-sm font-bold text-[#1a1d2e]">{t.template_name}</h3>
                  <button onClick={() => deleteTemplate(t.id)} className="text-[10px] font-semibold text-red-500 hover:text-red-700 transition-colors flex-shrink-0 ml-2">
                    Delete
                  </button>
                </div>
                {t.variables && t.variables.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {(Array.isArray(t.variables) ? t.variables : []).map(v => (
                ...
                <p className="text-xs text-[#9196b0] leading-relaxed flex-1 whitespace-pre-wrap">{t.template_body}</p>
                <p className="text-[10px] text-[#9196b0] mt-3 pt-3 border-t border-[#e2e5f0]">
                  {t.created_at ? new Date(t.created_at).toLocaleDateString('de') : ''}
                </p>
              </div>
            ))}
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-5" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-[#1a1d2e]">New Template</h3>
            <form onSubmit={createTemplate} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[#9196b0] uppercase tracking-wide mb-1.5">Template Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-[#9196b0] uppercase tracking-wide mb-1.5">Body</label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-semibold text-[#9196b0] uppercase tracking-wide mb-1.5">Variables (comma separated)</label>
                <input
                  type="text"
